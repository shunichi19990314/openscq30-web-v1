...
- ~~EQ変更時の切断~~ → **根因特定・修正済み**（2026-09-24）:
  - 根因: SETパケットのDRCブロックにstate由来のゼロ塊（byte 0 = -12.0dB の無効値）を送っていたためファームウェアが接続破棄
  - 修正: v1/v2共通の `apply_drc()` 計算カーブを送信（v2公式 `set_equalizer_with_drc` と同一方式）
  - 回帰テスト: SETパケットのDRC欄が計算値かつ非ゼロであることを検証
...
- ~~ボタン設定（Phase 3）~~ → **実装済み**（2026-09-23）:
  ...
  - 実機実験(2026-09-24): 下位ニブル0=VolumeUpは実行されるが、上位ニブル=0xF の混合バイト(0xF0)は保存・ACKされるものの実行されない → **設定時は対称バイト (action<<4)|action を送信**（無効化=0xFF）
